...
def is_cube_in_surface(cube, cubes_positions, borders):
    surface = 0
...
def count_adjacent_sides(cube_positions, borders):
    counter = 0

    for cube in cube_positions:
        for i in [-1, 1]:
            if [cube[0]+i, cube[1], cube[2]] in cube_positions or is_cube_in_surface([cube[0]+i, cube[1], cube[2]], cube_positions, borders):
                counter += 1
            if [cube[0], cube[1]+i, cube[2]] in cube_positions or is_cube_in_surface([cube[0], cube[1]+i, cube[2]], cube_positions, borders):
                counter += 1
            if [cube[0], cube[1], cube[2]+i] in cube_positions or is_cube_in_surface([cube[0], cube[1], cube[2]+i], cube_positions, borders):
                counter += 1
...
def find_whole_surface(file_name):
    ...
    surface = len(cubes) * 6 - count_adjacent_sides(cubes, borders)
    # for cube in cubes:
    #     if is_cube_in_surface(cube, cubes, borders):
    #         surface += (6-count_adjacent_sides(cube, cubes))
